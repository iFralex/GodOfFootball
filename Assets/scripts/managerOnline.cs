...
public class managerOnline : Photon.PunBehaviour
{
    public GameObject aspettaConnessionePan;
    ...
    public static bool maschi => managerOffline.maschi;
    ...
    azioniFreccia frecciaDaID(int id)
    {
        foreach (azioniFreccia freccia in listaFrecce)
            if (freccia != null)
                if (freccia.GetComponentInChildren<PhotonView>().viewID == id)
                    return freccia;
        foreach (azioniFreccia freccia in listaFrecceAvver)
            if (freccia != null)
                if (freccia.GetComponentInChildren<PhotonView>().viewID == id)
                return freccia;
        return null;
    ...
    azioniFreccia frecciaOpposta(azioniFreccia freccia)
    {
        if (listaFrecce.Contains(freccia))
        {
            foreach (azioniFreccia f in listaFrecceAvver)
                if (f != null)
                {
                    if (f.name == freccia.name)
                        return f;
                }
                else
                    listaFrecceAvver.Remove(f);
            for (int i = 0; i < listaFrecceAvver[0].transform.parent.childCount; i++)
                if (listaFrecceAvver[0].transform.parent.GetChild(i).name == freccia.name)
                    return listaFrecceAvver[0].transform.parent.GetChild(i).GetComponent<azioniFreccia>();
        }
        else foreach (azioniFreccia f in listaFrecce)
            {
                if (f != null)
                {
                    if (f.name == freccia.name)
                    ...
                    listaFrecce.Remove(f);
                for (int i = 0; i < listaFrecce[0].transform.parent.childCount; i++)
                    if (listaFrecce[0].transform.parent.GetChild(i).name == freccia.name)
                        return listaFrecce[0].transform.parent.GetChild(i).GetComponent<azioniFreccia>();
            }
        print("lll");
        return freccia;
    }

    giocatore playerContrario(giocatore player)
    {
        giocatore[] g;
        if (playerÈCompagno(player))
            g = playersAvver;
        ...
        for (int i = 0; i < g.Length; i++)
            if (g[i].ruolo == player.ruolo && latoContrario(g[i].posizione) == player.posizione)
                return g[i];
        return null;
    }

    giocatore.Posizioni latoContrario(giocatore.Posizioni pos)
    {
        ...
        else
        {
            switch (pos)
            {
                case giocatore.Posizioni.sinistro:
                    return giocatore.Posizioni.destro;
                        case giocatore.Posizioni.destro:
                    return giocatore.Posizioni.sinistro;
                case giocatore.Posizioni.nulla:
                    return giocatore.Posizioni.nulla;
            }
            return giocatore.Posizioni.nulla;
        }
    }
    ...
    IEnumerator Inizia()
    {
        ...
        PhotonNetwork.player.SetCustomProperties(new ExitGames.Client.Photon.Hashtable() { { "pronto", true } });
        yield return new WaitWhile(() => !(bool)PhotonNetwork.otherPlayers[0].CustomProperties["pronto"]);
        Destroy(aspettaConnessionePan);
        ...
        foreach (PhotonPlayer p in PhotonNetwork.playerList)
            if (p != PhotonNetwork.player)
                nomeSquadraNemica = (string)p.CustomProperties["squadra"];
    ...
    {/*
        ...
        photonView.RPC("AzioniFrecciaOnline", PhotonTargets.Others, frecciaOpposta(freccia).GetComponentInChildren<PhotonView>().viewID);
        AzioniFrecciaOnline(freccia.GetComponentInChildren<PhotonView>().viewID);
    }

    [PunRPC]
    public void AzioniFrecciaOnline(int idFreccia)
    {
        print("dribing effettuato: " + driblingEffettuato + "  |  dribling nemico effettuato: " + driblingNemicoEffettuato + "  |  aereo: " + aereo + "  |  aereo nemico: " + aereoNemico);
        azioniFreccia freccia = frecciaDaID(idFreccia);
        if (timerAttivo)
        {
    ...
    {
        azioneAttiva = true;
        ld.eventSystem.gameObject.SetActive(false);
        ...
        if (PhotonNetwork.isMasterClient)
            photonView.RPC("Lancia", PhotonTargets.All, new Vector3(Random.Range(-10f, 10f), Random.Range(-10f, 10f), Random.Range(-10f, 10f)));
    ...
    IEnumerator OttieniRisultatoDribling(azioniFreccia freccia)
    {
        ...
        if (playerDestinatarioSpecifico(giocatore.Ruoli.centrocampista, playersContrari(proprietarioPalla)) != null)
        {
            if (PhotonNetwork.isMasterClient)
            {
                Vector3 a = new Vector3(Random.Range(-10f, 10f), Random.Range(-10f, 10f), Random.Range(-10f, 10f));
                Vector3 b = new Vector3(Random.Range(-10f, 10f), Random.Range(-10f, 10f), Random.Range(-10f, 10f));
                photonView.RPC("LanciaDoppio", PhotonTargets.Others, a, b);
                photonView.RPC("LanciaDoppio", PhotonTargets.MasterClient, b, a);
                print("a: " + a + "  b: " + b);
            }
    ...
    {
        photonView.RPC("TiroCrossOnline", PhotonTargets.All, n);
    }

    [PunRPC]
    public void TiroCrossOnline(int n)
    {
    ...
    IEnumerator Fuori(bool parata = false)
    {
        ...
        proprietarioPalla = playerPiùVicino(pos, g);
            ...
            proprietarioPalla = playerContrario(playerPiùVicino(pos, g));
        PosizionaPalla(proprietarioPalla);
        proprietarioPalla.AttivaFrecce();
        azioneAttiva = false;
        timerC = StartCoroutine(TimerAzione(2));
    ...
    IEnumerator CalcioDangolo()
    {
        void _Posiziona(giocatore _player)
        {
            ...
            if (playerÈCompagno(proprietarioPalla))
                ...
                aereoNemico = false;
            print(proprietarioPalla.ruolo + "  " + proprietarioPalla.posizione + "  " + playerÈCompagno(proprietarioPalla));
        }
        azioneAttiva = true;
        if (playerÈCompagno(proprietarioPalla))
            aereo = true;
        ...
        giocatore[] _players = playersContrari(proprietarioPalla);
        if (ris == 1 || ris == 2)
            _Posiziona(playerDestinatarioSpecifico(giocatore.Ruoli.terzino, _players, latoContrario(giocatore.Posizioni.sinistro)));
        else if (ris == 5 || ris == 6)
            _Posiziona(playerDestinatarioSpecifico(giocatore.Ruoli.terzino, _players, latoContrario(giocatore.Posizioni.destro)));
        else
            AttivaTiroCross();
        azioneAttiva = false;
        if (ris != 3 && ris != 4)
    ...
    {
        scegliLato.GetComponent<GraphicRaycaster>().enabled = playerÈCompagno(proprietarioPalla);
        print("tiro cross attivato");
        if (playerÈCompagno(proprietarioPalla))
            if (aereo)
                CardSpiegazione(Spiegazioni.colpoDiTesta);
            else
    ...
    IEnumerator DoppioDribling()
    {
        ...
        if (playerDestinatarioSpecifico(giocatore.Ruoli.difensore, playersContrari(proprietarioPalla), latoContrario(giocatore.Posizioni.sinistro)) != null)
        {
        ...
        void _RubaPalla()
        {
            CardSpiegazione(Spiegazioni.doppioDribling);
            CardSpiegazione(traduzioni.traduci("Palla rubata"));
            if (playerDestinatarioSpecifico(giocatore.Ruoli.difensore, playersContrari(proprietarioPalla), latoContrario(giocatore.Posizioni.destro)) == null)
            {
                if (playerDestinatarioSpecifico(giocatore.Ruoli.difensore, playersContrari(proprietarioPalla), latoContrario(giocatore.Posizioni.sinistro)) != null)
                    proprietarioPalla = playerDestinatarioSpecifico(giocatore.Ruoli.difensore, playersContrari(proprietarioPalla), latoContrario(giocatore.Posizioni.sinistro));
                else
                    proprietarioPalla = playerDestinatarioSpecifico(giocatore.Ruoli.terzino, playersContrari(proprietarioPalla), latoContrario(giocatore.Posizioni.sinistro));
            }
            else
            {
                if (playerDestinatarioSpecifico(giocatore.Ruoli.difensore, playersContrari(proprietarioPalla), latoContrario(giocatore.Posizioni.sinistro)) != null)
                    proprietarioPalla = proprietarioPalla = playerDestinatarioSpecifico(giocatore.Ruoli.difensore, playersContrari(proprietarioPalla), latoContrario(giocatore.Posizioni.sinistro));
                else
                    proprietarioPalla = playerDestinatarioSpecifico(giocatore.Ruoli.difensore, playersContrari(proprietarioPalla), latoContrario(giocatore.Posizioni.destro));
            }                
            PosizionaPalla(proprietarioPalla);
        ...
        void LancioDoppioOnline()
        {
            ld.risultati[0] = ld.risultati[1] = -1;
            if (PhotonNetwork.isMasterClient)
            {
                Vector3 a = new Vector3(Random.Range(-10f, 10f), Random.Range(-10f, 10f), Random.Range(-10f, 10f));
                Vector3 b = new Vector3(Random.Range(-10f, 10f), Random.Range(-10f, 10f), Random.Range(-10f, 10f));
        ...
        if (playerÈCompagno(proprietarioPalla))
        {
            ...
            if (ld.risultati[0] > ld.risultati[1])
            {
                ...
                else
                {
                    yield return new WaitForSeconds(3);
                    print("rigore");
                    giocatore pl = playerDestinatarioSpecifico(giocatore.Ruoli.difensore, playersContrari(proprietarioPalla), latoContrario(giocatore.Posizioni.destro));
                    pl.Fallo(true);
            ...
            else
            {
                print("punizione");
                yield return new WaitForSeconds(3);
                playerDestinatarioSpecifico(giocatore.Ruoli.difensore, playersContrari(proprietarioPalla), latoContrario(giocatore.Posizioni.sinistro)).Fallo(true);
                ...
                if (ld.risultato <= 2)
                {
                    ...
                    CardSpiegazione(Spiegazioni.punizione);
                    CardSpiegazione(traduzioni.traduci("Palla rubata"));
                    proprietarioPalla = playerDestinatarioSpecifico(giocatore.Ruoli.terzino, playersContrari(proprietarioPalla), latoContrario(giocatore.Posizioni.destro));
                    PosizionaPalla(proprietarioPalla);
                    proprietarioPalla.AttivaFrecce();
                    pannello.gameObject.SetActive(false);
                    azioneAttiva = false;
                ...
                else if (ld.risultato == 3 || ld.risultato == 4)
                {
                    ...
                    CardSpiegazione(traduzioni.traduci("Palla rubata"));
                    proprietarioPalla = playerDestinatarioSpecifico(giocatore.Ruoli.difensore, playersContrari(proprietarioPalla), latoContrario(giocatore.Posizioni.sinistro));
                    giocatore g = playerDestinatarioSpecifico(giocatore.Ruoli.difensore, playersContrari(proprietarioPalla), latoContrario(giocatore.Posizioni.sinistro));
                    if (g == null)
                        proprietarioPalla = playerDestinatarioSpecifico(giocatore.Ruoli.terzino, playersContrari(proprietarioPalla), latoContrario(giocatore.Posizioni.sinistro));
                    else
                        g = proprietarioPalla;
        ...
        else
        {
            ...
            if (ld.risultati[0] < ld.risultati[1])
            {
                ...
                else
                {
                    yield return new WaitForSeconds(3);
                    print("rigore");
                    playerDestinatarioSpecifico(giocatore.Ruoli.difensore, playersContrari(proprietarioPalla), latoContrario(giocatore.Posizioni.destro)).Fallo(true);
                    CardSpiegazione(Spiegazioni.rigore);
                    CardSpiegazione("+...");
                    if (PhotonNetwork.isMasterClient)
                        photonView.RPC("Lancia", PhotonTargets.All, new Vector3(Random.Range(-10f, 10f), Random.Range(-10f, 10f), Random.Range(-10f, 10f)));
    ...
    public IEnumerator TimerAzione(float aspetta = 0, bool _scelta = false)
    {
        ...
        if (playerÈCompagno(proprietarioPalla))
        {
            ...
            if (_scelta)
                ...
                photonView.RPC("PallaPersaOnline", PhotonTargets.All);
        }
    }

    [PunRPC]
    public void PallaPersaOnline()
    {
    ...
    public IEnumerator TimerPartita()
    {
        ...
        FinePartita();
        //photonView.RPC("FinePartita", PhotonTargets.AllViaServer);
    }
    ...
    public void FinePartita()
    {
        ...
        if (goal > goalAvver)
        {
            ...
            PlayerPrefs.SetInt("punti", PlayerPrefs.HasKey("punti") ? PlayerPrefs.GetInt("punti") + 6 : 3);
            PlayerPrefs.SetInt("vittorie", PlayerPrefs.HasKey("vittorie") ? PlayerPrefs.GetInt("vittorie") + 1 : 1);
        }
        ...
        else
        {
            ...
            PlayerPrefs.SetInt("parità", PlayerPrefs.HasKey("parità") ? PlayerPrefs.GetInt("parità") + 1 : 1);
        }
